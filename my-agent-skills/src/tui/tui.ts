...
export class TUI {
  ...
  constructor(options: TUIOptions) {
    ...
    this.themeManager = options.themeManager ?? new ThemeManager("dark")
  ...
  private async showHelp(): Promise<void> {
    ...
    console.log(`
${color("┌────────────────────────────────────────────────────────────┐", theme.border)}
${color("│", theme.border)}  ${color("Commands - My Agent Phase 5", theme.assistantPrefix)}                         ${color("│", theme.border)}
${color("├────────────────────────────────────────────────────────────┤", theme.border)}
${color("│", theme.border)}  ${color("Session Commands:", theme.statusBarHighlight)}                                        ${color("│", theme.border)}
...
${color("├────────────────────────────────────────────────────────────┤", theme.border)}
${color("│", theme.border)}  ${color("Advanced Commands:", theme.statusBarHighlight)}                                      ${color("│", theme.border)}
${color("│", theme.border)}  ${color("/theme <name>", theme.statusBarHighlight)}     Change theme (dark/light/monokai/nord)     ${color("│", theme.border)}
${color("│", theme.border)}  ${color("/compact", theme.statusBarHighlight)}        Show context compaction info               ${color("│", theme.border)}
${color("│", theme.border)}  ${color("/stats", theme.statusBarHighlight)}         Show session statistics                    ${color("│", theme.border)}
${color("│", theme.border)}  ${color("/queue", theme.statusBarHighlight)}         Show message queue status                  ${color("│", theme.border)}
${color("├────────────────────────────────────────────────────────────┤", theme.border)}
...
`)
  ...
  private changeTheme(themeName?: string): void {
    if (!themeName) {
      ...
      this.addMessage({
        role: "assistant",
        content: `Available themes: ${themes.join(", ")}\nCurrent: ${this.themeManager.getThemeName()}\nUsage: /theme <name>`,
      })
      this.render()
      return
    }
    ...
    if (this.themeManager.setTheme(themeName)) {
      ...
      this.addMessage({
        role: "assistant",
        content: `Unknown theme: ${themeName}. Available: ${this.themeManager.getAvailableThemes().join(", ")}`,
      })
    }
    this.render()
  }
  ...
  // 显示压缩信息
  private async showCompactionInfo(): Promise<void> {
    const compactor = new ContextCompactor()
    
    // 简单显示信息
    this.addMessage({
      role: "assistant",
      content: `Context Compaction Info:
- Max messages: ${compactor.getMaxTokens() / 100} (estimated)
- Compaction is enabled
- Older messages are summarized when context exceeds limits`,
    })
    this.render()
  }
  ...
  private showSessionStats(): void {
    ...
    console.clear()
    console.log(color("\n┌────────────────────────────────────────────────────────────┐", theme.border))
    console.log(color("│", theme.border) + color(" Session Statistics".padEnd(60) + "│", theme.assistantPrefix))
    console.log(color("├────────────────────────────────────────────────────────────┤", theme.border))
    
    // 基本信息
    if (this.session) {
      const entries = this.session.entries.length
      const userMsgs = this.session.entries.filter(e => e.type === "user").length
      const assistantMsgs = this.session.entries.filter(e => e.type === "assistant").length
      const toolMsgs = this.session.entries.filter(e => e.type === "tool").length
      
      console.log(color("│", theme.border) + color(` Session: ${this.session.meta.name}`.padEnd(60) + "│", theme.statusBar))
      console.log(color("│", theme.border) + color(` Total entries: ${entries}`.padEnd(60) + "│", theme.statusBar))
      console.log(color("│", theme.border) + color(`   User messages: ${userMsgs}`.padEnd(60) + "│", theme.statusBar))
      console.log(color("│", theme.border) + color(`   Assistant messages: ${assistantMsgs}`.padEnd(60) + "│", theme.statusBar))
      console.log(color("│", theme.border) + color(`   Tool calls: ${toolMsgs}`.padEnd(60) + "│", theme.statusBar))
    }
    ...
    const cmdCount = this.options.extensionManager?.getCommands().length ?? 0
    
    console.log(color("├────────────────────────────────────────────────────────────┤", theme.border))
    console.log(color("│", theme.border) + color(" Extensions & Tools".padEnd(60) + "│", theme.assistantPrefix))
    console.log(color("├────────────────────────────────────────────────────────────┤", theme.border))
    console.log(color("│", theme.border) + color(` Extensions loaded: ${extCount}`.padEnd(60) + "│", theme.statusBar))
    console.log(color("│", theme.border) + color(` Total tools: ${toolCount}`.padEnd(60) + "│", theme.statusBar))
    console.log(color("│", theme.border) + color(` Total commands: ${cmdCount}`.padEnd(60) + "│", theme.statusBar))
    
    // 主题信息
    console.log(color("├────────────────────────────────────────────────────────────┤", theme.border))
    console.log(color("│", theme.border) + color(" Current Theme".padEnd(60) + "│", theme.assistantPrefix))
    console.log(color("├────────────────────────────────────────────────────────────┤", theme.border))
    console.log(color("│", theme.border) + color(` Theme: ${this.themeManager.getThemeName()}`.padEnd(60) + "│", theme.statusBar))
    console.log(color("│", theme.border) + color(` Available: ${this.themeManager.getAvailableThemes().join(", ")}`.padEnd(60) + "│", theme.statusBar))
  ...
  private showQueueStatus(): void {
    this.addMessage({
      role: "assistant",
      content: `Message Queue Status:
- Steering messages: queued for delivery after current tool
- Follow-up messages: queued for delivery after agent is idle
Use Enter to send steering, Alt+Enter to queue follow-up (in raw mode)`,
    })
    this.render()
  }
  ...
  private welcome(): void {
    ...
    const extCount = this.options.extensionManager?.listExtensions().length ?? 0
    const toolCount = this.options.extensionManager?.getTools().length ?? 4
    const currentTheme = this.themeManager.getThemeName()

    console.log(`
${color("┌────────────────────────────────────────────────────────────┐", theme.border)}
${color("│", theme.border)}  ${color("My Agent Phase 5 - Advanced Features", theme.assistantPrefix)}                     ${color("│", theme.border)}
${color("├────────────────────────────────────────────────────────────┤", theme.border)}
${color("│", theme.border)}  ${color("Features:", theme.statusBarHighlight)}                                                   ${color("│", theme.border)}
${color("│", theme.border)}    • Message queue (steering/follow-up)                           ${color("│", theme.border)}
${color("│", theme.border)}    • Context compaction                                       ${color("│", theme.border)}
${color("│", theme.border)}    • Theme system (dark/light/monokai/nord)                    ${color("│", theme.border)}
${color("│", theme.border)}    • Streaming support                                        ${color("│", theme.border)}
${color("├────────────────────────────────────────────────────────────┤", theme.border)}
${color("│", theme.border)}  ${color(`Extensions: ${extCount}  |  Tools: ${toolCount}  |  Theme: ${currentTheme}`, theme.statusBar)}  ${color("│", theme.border)}
